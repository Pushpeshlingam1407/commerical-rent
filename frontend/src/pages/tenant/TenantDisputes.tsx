...
export const TenantDisputes: React.FC = () => {
  const { user } = useAuth();
  ...
  const fetchData = async () => {
    try {
      ...
      const [leasesRes, disputesRes] = await Promise.all([
        api.get('/leases'),
        api.get('/disputes'),
      ]);

      // Filter leases for this tenant (match by email) that are ACTIVE or APPROVED
      const userLeases = leasesRes.data.filter((l: any) => 
        l.tenant === user.email && (l.leaseStatus === 'ACTIVE' || l.leaseStatus === 'APPROVED')
      );
      setLeases(userLeases);

      // Filter disputes raised by this tenant
      const userDisputes = disputesRes.data.filter((d: any) => d.raisedByEmail === user.email);
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Track disputes you've raised on your leases
          </Typography>
        </Box>
        <Button
          variant="contained"
      ...
      {loading ? (
        ...
        >
          ...
          <Typography color="text.secondary">
            No disputes raised yet. Good work!
          </Typography>
        </Paper>
      ) : (
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ mt: 2 }}>
          {activeLeases.length === 0 ? (
            <Alert severity="info">
              You need to have an active lease to raise a dispute.
            </Alert>
          ) : (
